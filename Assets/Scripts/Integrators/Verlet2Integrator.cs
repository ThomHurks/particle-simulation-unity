public class Verlet2Integrator : Integrator
{
    ...
    private double[] derivativeIPlusOneHalve;
    ...
    public override void Initialize(ParticleSystem a_ParticleSystem)
    {
        ...
        derivativeIPlusOneHalve = new double[particleDimensions];
    ...
    public override void Step(ParticleSystem a_ParticleSystem, float a_DeltaTime)
    {
        ...
        a_ParticleSystem.ParticleDerivative(derivativeIPlusOneHalve); // v+1,a+1
        for (int i = 0; i < n; i++)
        {
            int i4 = i * 4;
            stateStorage[i4 + 2] = originals[i4 + 2] + (derivativeIPlusOneHalve[i4 + 2] + derivativeI[i4 + 2]) * a_DeltaTime / 2f;
            stateStorage[i4 + 3] = originals[i4 + 3] + (derivativeIPlusOneHalve[i4 + 3] + derivativeI[i4 + 3]) * a_DeltaTime / 2f;
        }
